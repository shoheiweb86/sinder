...
@section('content')
  ...
  $('#seeking_thumbnail').change(function(e) {
    ...
    if (file) {
      ...
      $('#seeking_thumbnail-preview').hide(); // 画像プレビューを非表示にする
      $('#js-clear-button').hide(); // バツボタンを非表示にする
    }
    hideThumbnailError(); // バツボタンを押した際にエラーメッセージを非表示にする
  });

  if ($('#seeking_thumbnail-preview').attr('src') !== "{{ asset('default-thumbnail.png') }}") {
    $('#seeking_thumbnail-preview').show(); // 画像プレビューを表示する
    $('#js-clear-button').show(); // バツボタンを表示する
  } else {
    $('#seeking_thumbnail-preview').hide(); // 画像プレビューを非表示にする
  ...
  $('#js-clear-button').click(function(e) {
    ...
    $('#seeking_thumbnail-preview').attr('src', "{{ asset('default-thumbnail.png') }}");
    $('#seeking_thumbnail-preview').hide(); // 画像プレビューを非表示にする
    $('#js-clear-button').hide(); // バツボタンを非表示にする
    hideThumbnailError(); // バツボタンを押した際にエラーメッセージを非表示にする
  });
